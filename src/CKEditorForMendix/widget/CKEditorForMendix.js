    ...
    return declare("CKEditorForMendix.widget.CKEditorForMendix", [_WidgetBase, _TemplatedMixin], {
        ...
        _createChildNodes: function(callback) {
            ...
            function dataFeed(opts, callback) {
              ...
              function dataFeedTag(opts, callback) {
              ...
              // Mention 
              this._executeMf(this._contextObj, this.mentionListMicroflow, lang.hitch(this, function (obj) {
                if (!!obj) {
                    for (let i = 0; i < obj.length; i++) {
                        const element = obj[i];
                        if (!!element.jsonData &&
                            !!element.jsonData.attributes ) {
                                const attribute = element.jsonData.attributes;
                                const fullName = (!!attribute.FullName)? attribute.FullName .value:'';
                                const email = (!!attribute.Email)? attribute.Email .value:'';
                                const name = (!!attribute.Name)? attribute.Name .value:'';
                                const Base64Image = (!!attribute.Base64Image)? attribute.Base64Image.value : '';
                                const userObj = {
                                    id: uuidv4(),
                                    fullname: fullName,
                                    username: email,
            ...
            var self = this;
             function  getTags(){
                var tagURL = "";
                if (!!self.tagURLMicroflow){
                    self._executeMf(self._contextObj, self.tagURLMicroflow, lang.hitch(self, function (obj) {
                        if (!!obj) {
                            tagURL = obj[0].jsonData.attributes.TagURL.value;
                        }
                        getTagList(tagURL);
            ...
            // Tag
            function getTagList(tagURL) {

                self._executeMf(self._contextObj, self.tagListMicroflow, lang.hitch(self, function (obj)  {
                    if (!!obj) {
                        for (let i = 0; i < obj.length; i++) {
                            const element = obj[i];
                            if (!!element.jsonData &&
                                !!element.jsonData.attributes ) {
                                    const attribute = element.jsonData.attributes;
                                    const name = (!!attribute.Name)? attribute.Name .value:'';
                                    const description = (!!attribute.Description)? attribute.Description .value:'';
                                    const tagObj = {
                                        id: uuidv4(),
                                        name: name,
            ...
            var config = {
                ...
                mentions: [{
                    ...
                    itemTemplate: '<li data-id="{id}">' +
                      ...
                      '</li>',
                    outputTemplate: '<a href="mailto:{username}">@{fullname}</a><span>&nbsp;</span>',
                    minChars: 0
                  },
                  {
                    feed: dataFeedTag,
                    ...
                    '<span class="fullname">{description}</span>' +
                    '</li>',
                    outputTemplate: '<a href="{TagURLValue}{name}">{name}</a><span>&nbsp;</span>',
                    minChars: 0
                  }
                ],
            };
